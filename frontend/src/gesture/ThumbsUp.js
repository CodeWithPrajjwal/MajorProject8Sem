...
export const checkThumbsUp = (landmarks) => {
  ...
  // All fingers folded except thumb, and thumb pointing up
  const isThumbUp = thumbTipY < thumbIPY;
  const isOtherFingersDown = [indexMCPY, middleMCPY, ringMCPY, pinkyMCPY].every(
    (joint) => thumbTipY < joint
  );

  const result = isThumbUp && isOtherFingersDown;
